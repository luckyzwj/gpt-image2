...
function getMimeFromFormat(format: string) {
...
/**
 * Upload base64 image output to storage.
 * Falls back to an inline data URL when storage is not configured.
 */
export async function uploadBase64Asset({
  ...
  if (!isR2Configured()) {
    return {
      publicUrl: `data:${mimeType};base64,${normalizedBase64}`,
      storageKey: "",
      mimeType,
  ...
  const storageKey = `studio/${kind}/${userId}/${timestamp}_${random}.${extension}`;
  const publicUrl = await uploadToR2(storageKey, buffer, mimeType);
