...
const articlePlanSchema = z.object({
  ...
  body: z.string().min(1, "Article body is required").max(20000),
...
export async function POST(req: NextRequest) {
  try {
    ...
    const payload = articlePlanSchema.parse(await req.json());
    ...
    return NextResponse.json(
      { error: getErrorMessage(error, "Failed to create article plan task") },
      { status: 500 },
    );
  }
}
