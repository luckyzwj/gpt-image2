...
export async function POST(req: NextRequest) {
  try {
    ...
    const payload = creationPlanSchema.parse(await req.json());
    ...
    const result = await createStudioTask({
      ...
      requestPayload: {
        ...
        scenario: payload.scenario || "standard",
    ...
    return NextResponse.json({ error: getErrorMessage(error, "Failed to create creation plan task") }, { status: 500 });
  }
}
