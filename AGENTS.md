...
- User uploads enter through `app/api/uploads/reference/route.ts` (multi-file, writes a `studio_asset` row per file).
  - The legacy single-file routes `app/api/upload/image/route.ts` and `app/api/upload/simple/route.ts` were deprecated 2026-05-23 and now return HTTP 410 Gone with a `migrateTo` hint. They will be physically removed after one release cycle.
...
- `app/api/upload/simple/route.ts` and `app/api/upload/image/route.ts` are deprecated stubs returning 410 Gone — do not extend them. New upload work goes through `/api/uploads/reference`.
